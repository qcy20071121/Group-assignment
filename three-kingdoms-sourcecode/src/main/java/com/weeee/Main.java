...
public class Main {
            ...
            switch (c) {
                ...
                case 2: {
                    ...
                    while (!back) {
                        ...
                        System.out.println("4. Custom Search Generals (Multi-criteria)");
                        System.out.println("5. Interpolation Search General");
                        ...
                        switch (choice2) {
                            ...
                            case 4: {
                                System.out.println("\n--- Custom Search (Leave blank/press Enter to skip any criteria) ---");
                                System.out.print("Enter name query: ");
                                String nameQ = sc.nextLine().trim();
                                if (nameQ.isEmpty()) nameQ = null;
                                
                                System.out.print("Enter troop type (Archer/Cavalry/Infantry/All): ");
                                String typeQ = sc.nextLine().trim();
                                if (typeQ.isEmpty() || typeQ.equalsIgnoreCase("all")) typeQ = null;

                                Integer minStr = readOptionalInt(sc, "Enter minimum strength: ");
                                Integer maxStr = readOptionalInt(sc, "Enter maximum strength: ");
                                Integer minLdr = readOptionalInt(sc, "Enter minimum leadership: ");
                                Integer maxLdr = readOptionalInt(sc, "Enter maximum leadership: ");
                                Integer minInt = readOptionalInt(sc, "Enter minimum intelligence: ");
                                Integer maxInt = readOptionalInt(sc, "Enter maximum intelligence: ");
                                Integer minPol = readOptionalInt(sc, "Enter minimum politic: ");
                                Integer maxPol = readOptionalInt(sc, "Enter maximum politic: ");
                                Integer minHp = readOptionalInt(sc, "Enter minimum hitpoint: ");
                                Integer maxHp = readOptionalInt(sc, "Enter maximum hitpoint: ");

                                List<Officer> matches = SortSearch.customSearch(generals, nameQ, typeQ, 
                                        minStr, maxStr, minLdr, maxLdr, minInt, maxInt, minPol, maxPol, minHp, maxHp);

                                System.out.println("\nFound " + matches.size() + " matches:");
                                for (Officer o : matches) {
                                    System.out.print(o);
                                }
                                break;
                            }
                            case 5: {
                                System.out.print("Enter attribute to search (strength/leadership/intelligence/politic/hitpoint): ");
                                String attr = sc.nextLine().trim();
                                System.out.print("Enter target value for Interpolation Search: ");
                                int targetVal = sc.nextInt();
                                sc.nextLine();
                                List<Officer> copy = new ArrayList<>(generals);
                                Officer found = SortSearch.interpolationSearch(copy, targetVal, attr);
                                if (found != null) {
                                    System.out.println("\nFound General (via Interpolation Search): " + found);
                                } else {
                                    System.out.println("\nNo General found with " + attr + " = " + targetVal);
                                }
                                break;
    ...
    private static Integer readOptionalInt(Scanner sc, String prompt) {
        ...
        try {
            ...
            System.out.println("Invalid number format. Skipping this criteria.");
            return null;
        }
    }
}